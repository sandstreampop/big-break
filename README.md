...
**Both games ship from `main`, one workflow, one artifact.** Push to `main` →
CI builds `dist/`, runs every gate above, and (only if green) deploys the whole
`dist/` to GitHub Pages. The music game lands at the root, the mystery game at
`/mystery/`, sharing the compiled JS. No `gh-pages` branch, no second deployer.
