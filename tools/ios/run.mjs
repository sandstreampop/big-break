// iOS (WebKit) harness runner.
//
//   node tools/ios/run.mjs                 # full iPhone matrix × probes
//   node tools/ios/run.mjs --quick         # iPhone 15 Pro only (fast PR smoke)
//   node tools/ios/run.mjs --device=iphone-se
//   node tools/ios/run.mjs --probe=zoom-defense-wired
//   node tools/ios/run.mjs --strict        # also fail if a known-bug is now fixed (promote it)
//
// Exit code: non-zero if any REGRESSION GUARD (must-pass) fails — that is the CI
// gate. Known bugs that still reproduce are reported but do NOT fail the build; a
// known bug that has started passing is flagged for promotion. Same contract as
// tools/android/run.mjs — mirror the contract, not the code (handoff §5).
//
// If the WebKit browser binary isn't installed (the download is a separate step
// from `npm ci`; some sandboxes can't fetch it), the DEVICE probes report as SKIP
// with a reason and the STATIC probes still run — never a false green, never a
// hard crash. CI installs WebKit, so there the device probes run for real.
...
import {
  serve, collectErrors, DIST, ROOT,
  webkitAvailable, launchBrowser, newDevicePage,
} from './harness.mjs';
...
async function main() {
  ...
  if (QUICK) matrix = matrix.filter((d) => d.id === 'iphone-15-pro');
  ...
  const haveWebkit = webkitAvailable();
  const server = await serve(DIST);
  console.log(paint(C.bold, '\nBIG BREAK — iOS (WebKit) harness') + paint(C.dim, `  (serving dist/ at ${server.origin})`));
  console.log(paint(C.dim, `engine: WebKit${haveWebkit ? '' : ' (NOT INSTALLED — device probes will SKIP; run `npx playwright install webkit`)'}`));
  ...
  try {
    ...
    for (const p of probes.filter((x) => x.scope === 'static')) {
      ...
      record({ probe: p.id, title: p.title, scope: 'static', expectation: p.expectation, status: classify(p.expectation, threw), error: threw });
    }

    // ---- manual probes: report as documented SKIPs ----
    for (const p of probes.filter((x) => x.expectation === 'manual')) {
      record({ probe: p.id, title: p.title, expectation: 'manual', status: SKIP, why: p.why });
    }

    // ---- device probes × matrix ----
    const devProbes = probes.filter((x) => x.scope === 'device' && x.expectation !== 'manual');
    if (devProbes.length && !haveWebkit) {
      // WebKit binary absent: report each device probe as a SKIP with a reason,
      // once (device-agnostic — the reason is the same on every device).
      for (const p of devProbes) {
        record({ probe: p.id, title: p.title, expectation: p.expectation, status: SKIP,
          why: 'WebKit browser not installed — run `npx playwright install webkit` (CI does this).' });
      }
    } else if (devProbes.length) {
      browser = await launchBrowser();
      for (const device of matrix) {
        for (const p of devProbes) {
          let ctx, page, threw = null;
          try {
            ({ ctx, page } = await newDevicePage(browser, device));
          ...
          record({ probe: p.id, title: p.title, device: device.id, expectation: p.expectation, status: classify(p.expectation, threw), error: threw });
        }
      }
    }
  } finally {
  ...
  if (proven.length) {
    console.log(paint(C.yellow, `\nKnown iOS bugs reproduced (expected — prove-before-fix):`));
  ...
  await writeFile(join(ROOT, 'tools', 'ios', 'report.json'),
    JSON.stringify({ generatedAgainst: 'dist/', engine: 'webkit', webkitAvailable: haveWebkit, devices: matrix.map((d) => d.id), results }, null, 2));
