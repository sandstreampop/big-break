...
// The two shipping genres plus the probe. The probe is a deliberately minimal
// pack (one stat, one resource, one path, a tiny deck, zero subsystems) whose
// only job is to be the executable definition of genre-neutrality: if the
// engine can run it, the core carries no music shape. It is registered here so
// the invariants sweep it alongside the real packs.
