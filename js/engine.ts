...
//
//   • createEngine(pack) — an ISOLATED engine instance bound to one pack.
//     Instances share no pack state, so sims, tests, multi-pack pages, and
//     workers can run different genres side by side without a global handoff.
//
//   • the module-level functions (drawNextCard, resolveSwipe, advance, …) —
//     the classic surface the UI shell and tools use. They delegate to a
//     DEFAULT instance selected by useContentPack()/newRun(pack, …), exactly
//     the old "active pack" semantics, byte-identical behavior.
//
// The bridge between them: pack PLUGINS call module-level services (findEvent,
// requiresOk, applyEffects, perkFlag, actLength) rather than receiving an
// engine handle. Every instance method therefore runs with the instance bound
// as the CALL-SCOPED current engine, so a plugin's synchronous callback into
// the module surface resolves to the engine that invoked it — not to whatever
// pack happens to be the session default. JS is single-threaded and every
...
// test time instead: the pack-contract validator (js/validate.ts) and the
// cross-pack invariant assert every declared gate key resolves, so a typo
...
// An isolated engine bound to one pack — the whole pack-dependent surface.
// createEngine() is the constructor; the module-level functions below delegate
// to a default instance for the classic single-active-pack callers.
export type Engine = ReturnType<typeof createEngine>;

// The call-scoped current engine: while any instance method runs, module-level
// services resolve to THAT instance, so a plugin calling back into the module
// surface (findEvent, requiresOk, applyEffects, …) reads the pack that invoked
// it. Restored on exit (including throws); null outside any engine call.
let CURRENT: Engine | null = null;
// The session default: what useContentPack()/newRun(pack, …) selected. The
// module-level surface uses it whenever no instance call is on the stack.
let DEFAULT: Engine | null = null;

function active(): Engine {
  const e = CURRENT ?? DEFAULT;
  if (!e) {
    throw new Error('big-break: no active pack — call useContentPack(pack) (or use createEngine(pack)) before any engine call.');
  }
  return e;
}

// Build an isolated engine for `pack`. Everything the old module-global PACK
// version did, with the global replaced by this closure — behaviorally
// byte-identical (the goldens pin it).
export function createEngine(pack: Pack) {
  const PACK = pack;

  // Tutorial cards live outside PACK.events so they can never enter normal
  // decks; chains and resume still need to find them by id.
  function findEvent(id: string): GameEvent | null {
  ...
  function actLength(state: RunState, act: number) {
  ...
  // The pack's core stats. Order is load-bearing — it fixes the order stat
  // deltas are recorded, which the goldens pin — so a pack must keep it stable.
  const stats = () => PACK.manifest.stats;

  // ---------- Run lifecycle ----------

  function offerLoadouts(unlockedLoadoutIds: string[], rng: () => number = Math.random) {
  ...
  function newRun(loadoutId: string, unlockedPacks: string[], rng: () => number = Math.random, perks: string[] = []) {
  ...
  function newTutorialRun(rng = Math.random) {
    const t = PACK.tutorialStart;
    const state = newRun(t?.loadout ?? PACK.loadouts[0].id, [], rng, []);
    state.tutorial = true;
  ...
  function applyComeback(state: RunState) {
    PACK.comeback?.(state);
  }

  // Loadout mastery (earned across runs): +level to every core stat
  function applyMastery(state: RunState, level: number) {
  ...
  // The merged predicate registry for this pack — each plugin's `requires` map
  // contributes its keys, and requiresOk dispatches a non-neutral key here.
  // Memoized per instance: building it is cheap but this sits in the deck hot
  // path.
  let _reqPreds: Record<string, (state: any, arg: any) => boolean> | null = null;
  function requiresPredicates(): Record<string, (state: any, arg: any) => boolean> {
    if (_reqPreds) return _reqPreds;
  ...
  function requiresOk(r: Requires | null | undefined, state: RunState) {
    if (!r) return true;
  ...
  function eligibleEvents(state: RunState) {
  ...
  function drawNextCard(state: RunState, rng: () => number = Math.random) {
  ...
  // Fire a lifecycle hook across the pack's plugins. Empty/absent registry
  ...
  function activePerks(state: RunState): PerkDef[] {
  ...
  function perkFlag(state: RunState, key: string): boolean {
  ...
  function rollComponents(state: RunState, choice: Choice, opts: any = {}) {
  ...
  function resolveSwipe(state: RunState, side: Side, rng: () => number = Math.random, opts: any = {}) {
    const ev = findEvent(state.currentEventId!)!;
  ...
  function applyEffects(state: RunState, effects: any, ev: GameEvent | null, choice: Choice | null, rng: () => number, tier?: any, appliedAccessories: any[] = [], mg: any = null): any[] {
    const deltas: any = [];
    const inst = PACK.loadoutById(state.loadout);
  ...
  // climax card, then ends in the finale. Returns the next flow step (see the
  // AdvanceStep union): card | crossroads | actStart | finale | gameover |
  // tutorialEnd.
  function advance(state: RunState): AdvanceStep {
  ...
  function commitPath(state: RunState, pathId: string) {
    state.path = pathId;
  ...
  function evaluateFinale(state: RunState) {
    state.phase = 'ended';
  ...
  function legacyPoints(state: RunState) {
    const s = state.stats;
  ...
  // no subsystem.
  function runSummary(state: RunState) {
  ...
  // ── The instance surface. Every method runs with this engine bound as the
  // call-scoped CURRENT, so a plugin's synchronous callback into the module
  // surface resolves here — restored even on throw. ──
  const self = {
    pack: PACK,
    findEvent: scoped(findEvent),
    actLength: scoped(actLength),
    offerLoadouts: scoped(offerLoadouts),
    newRun: scoped(newRun),
    newTutorialRun: scoped(newTutorialRun),
    applyComeback: scoped(applyComeback),
    applyMastery: scoped(applyMastery),
    requiresOk: scoped(requiresOk),
    eligibleEvents: scoped(eligibleEvents),
    drawNextCard: scoped(drawNextCard),
    perkFlag: scoped(perkFlag),
    rollComponents: scoped(rollComponents),
    incredibleTargets: scoped(incredibleTargets),
    choiceOdds: scoped(choiceOdds),
    resolveSwipe: scoped(resolveSwipe),
    applyEffects: scoped(applyEffects),
    advance: scoped(advance),
    commitPath: scoped(commitPath),
    checkFailStates: scoped(checkFailStates),
    evaluateFinale: scoped(evaluateFinale),
    legacyPoints: scoped(legacyPoints),
    runSummary: scoped(runSummary),
  };

  // Bind a method's dynamic extent to this instance (see CURRENT above).
  function scoped<A extends any[], R>(fn: (...args: A) => R): (...args: A) => R {
    return (...args: A): R => {
      const prev = CURRENT;
      CURRENT = self as unknown as Engine;
      try {
        return fn(...args);
      } finally {
        CURRENT = prev;
...
// ---------- The module-level surface (the classic single-active-pack API) ----------
// One session runs one active pack; these delegate to the DEFAULT instance
// selected by useContentPack()/newRun(pack, …). Pack plugins also call these as
// services — inside any instance call they resolve to the CALLING instance
// (see CURRENT above), so two engines never bleed into each other.

// Reuse the default instance when the pack hasn't changed (keeps the memoized
// requires-predicate registry warm — same behavior the module-global had).
function engineFor(pack: Pack): Engine {
  if (DEFAULT && DEFAULT.pack === pack) return DEFAULT;
...
// subsystem needs no stub.
export function useContentPack(pack: Pack): void {
  DEFAULT = engineFor(pack);
}
export function activePack(): Pack {
  return active().pack;
}

// Exported as a plugin-facing service so a subsystem (seeds) can inspect the
// deck without importing it.
export function findEvent(id: string): GameEvent | null {
  return active().findEvent(id);
}
export function actLength(state: RunState, act: number): number {
  return active().actLength(state, act);
}
export function offerLoadouts(unlockedLoadoutIds: string[], rng: () => number = Math.random) {
  return active().offerLoadouts(unlockedLoadoutIds, rng);
}
// newRun(pack, …) selects `pack` as the session default, exactly as the old
// module-global assignment did, then constructs the run on that instance.
export function newRun(pack: Pack, loadoutId: string, unlockedPacks: string[], rng: () => number = Math.random, perks: string[] = []) {
  DEFAULT = engineFor(pack);
  return DEFAULT.newRun(loadoutId, unlockedPacks, rng, perks);
}
export function newTutorialRun(pack: Pack, rng = Math.random) {
  DEFAULT = engineFor(pack);
  return DEFAULT.newTutorialRun(rng);
}
export function applyComeback(state: RunState): void {
  active().applyComeback(state);
}
export function applyMastery(state: RunState, level: number): void {
  active().applyMastery(state, level);
}
// Evaluate a Requires gate — a plugin-facing service (a subsystem can test its
// own conditions with the same semantics).
export function requiresOk(r: Requires | null | undefined, state: RunState): boolean {
  return active().requiresOk(r, state);
}
export function eligibleEvents(state: RunState): GameEvent[] {
  return active().eligibleEvents(state);
}
export function drawNextCard(state: RunState, rng: () => number = Math.random): GameEvent | null {
  return active().drawNextCard(state, rng);
}
// True if any active perk carries a truthy value at `key` — a plugin-facing
// service (the gear plugin reads its keepGearOnBad through it).
export function perkFlag(state: RunState, key: string): boolean {
  return active().perkFlag(state, key);
}
// Roll components for a choice; used by both resolution and the risk tell.
export function rollComponents(state: RunState, choice: Choice, opts: any = {}) {
  return active().rollComponents(state, choice, opts);
}
// The keys an INCREDIBLE payload scales — exported so the cross-pack symmetry
// invariant guards the real engine set, not a copy of it.
export function incredibleTargets(): string[] {
  return active().incredibleTargets();
}
// Risk tell: probability each tier fires, given uniform jitter.
export function choiceOdds(state: RunState, choice: Choice, opts: any = {}) {
  return active().choiceOdds(state, choice, opts);
}
export function resolveSwipe(state: RunState, side: Side, rng: () => number = Math.random, opts: any = {}) {
  return active().resolveSwipe(state, side, rng, opts);
}
// Applies an effects payload; returns display deltas [{key, amount}]. Exported
// so a plugin can apply its own payload through the full resolution pipeline.
export function applyEffects(state: RunState, effects: any, ev: GameEvent | null, choice: Choice | null, rng: () => number, tier?: any, appliedAccessories: any[] = [], mg: any = null): any[] {
  return active().applyEffects(state, effects, ev, choice, rng, tier, appliedAccessories, mg);
}
export function advance(state: RunState): AdvanceStep {
  return active().advance(state);
}
export function commitPath(state: RunState, pathId: string): string[] {
  return active().commitPath(state, pathId);
}
export function checkFailStates(state: RunState): string | null {
  return active().checkFailStates(state);
}
export function evaluateFinale(state: RunState) {
  return active().evaluateFinale(state);
}
export function legacyPoints(state: RunState): number {
  return active().legacyPoints(state);
}
export function runSummary(state: RunState): Record<string, any> {
  return active().runSummary(state);
}
